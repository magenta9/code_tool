...
export function buildDevCommands(port, includeElectron = true) {
    const commands = [
        "pnpm --filter @codetool/shared dev",
        "pnpm --filter @codetool/preload dev",
        "pnpm --filter @codetool/main dev",
        `pnpm --filter @codetool/renderer exec vite --port ${port}`
    ];
...
async function runElectronWhenReady(env) {
    const requestedPort = parseRendererPort(env.CODETOOL_RENDERER_PORT);
    const rendererUrl = env.CODETOOL_RENDERER_URL ?? getRendererUrl(requestedPort);
    const waitResult = await runPnpm(buildWaitOnArgs(rendererUrl), {
        ...env,
        CODETOOL_RENDERER_URL: rendererUrl,
        CODETOOL_RENDERER_PORT: String(requestedPort)
    });
...
async function runDevSession({ watchOnly }) {
    const preferredPort = parseRendererPort(process.env.CODETOOL_RENDERER_PORT);
    const rendererPort = await findAvailablePort(preferredPort);
    const rendererUrl = getRendererUrl(rendererPort);
    const env = {
        ...process.env,
        CODETOOL_RENDERER_PORT: String(rendererPort),
        CODETOOL_RENDERER_URL: rendererUrl
    };
